...
class UserActivityHelper:
    ...
    def format_user_activity(self, events: List[Dict], username: str) -> str:
        """Format user activity events into a natural language summary."""
        if not events:
            return f"No activity found for user {username}."
        
        if not self.openai_api_key:
            # Fallback to simple formatting if no OpenAI key
            summary = f"Found {len(events)} activities for {username}:\n"
        ...
        try:
            ...
            prompt = f"""
            Analyze the following AWS CloudTrail activities for user '{username}' and provide a concise summary:
